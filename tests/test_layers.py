...
from sylvestorch.layers import SylvesterBlock, AutoRecursiveFunction, GeneralizedSylvesterBlock
...
@pytest.mark.parametrize("batch,input_size", [(100, 3), 
                                              (100, 30), (100, 80), (1000,5)
                                              ])
def test_log_det_AutoRecursiveFunction_correct(batch, input_size):
    def jacobian(function, input):
        x = input.detach().requires_grad_(True)
        y, logdet = function(x)
    ...
    far = AutoRecursiveFunction(input_size).double()

    x = torch.randn(batch, input_size, dtype=torch.double, requires_grad=True)

    # Analytic output
    J, logdet = jacobian(far.forward, x)
    logdet = torch.sum(torch.log(torch.abs(logdet)), dim=1)
    detJ = torch.log(torch.abs(torch.det(J)))
    
    # The flow should be orientation-preserving often, but abs keeps this robust.
    assert torch.isfinite(logdet).all(), "Infinite values"
    assert not torch.isnan(logdet).any(), "NaN values"
    assert torch.allclose(detJ, logdet, atol=1e-4, rtol=1e-4), f"{detJ}\n{logdet}"

@pytest.mark.parametrize("batch,input_size", [(100, 3), 
                                              (100, 30), (100, 80), (1000,5)
                                              ])
def test_log_det_GeneralizedSylvesterBlock_correct(batch, input_size):
    def jacobian(function, input):
        x = input.detach().requires_grad_(True)
        y, logdet = function(x)
    ...
    block = GeneralizedSylvesterBlock(input_size).double()
    ...
    assert torch.isfinite(logdet).all(), "Infinite values"
    assert not torch.isnan(logdet).any(), "NaN values"
    assert torch.allclose(detJ, logdet, atol=1e-3, rtol=1e-4), f"{detJ}\n{logdet}"